...
async function migrate() {
    ...
    try {
        console.log('Starting comprehensive migration for tbl_admissions...');
        
        const [rows] = await conn.query('SHOW COLUMNS FROM tbl_admissions');
        ...
        const definitions = {
            ...
            'transfer_certificate_url': 'VARCHAR(255) DEFAULT NULL AFTER photo_url',
            'parent_name': 'VARCHAR(100) DEFAULT NULL AFTER student_name',
            'phone': 'VARCHAR(20) DEFAULT NULL AFTER parent_name',
            'status': 'VARCHAR(20) DEFAULT "PENDING" AFTER transfer_certificate_url'
        };

        for (const [col, def] of Object.entries(definitions)) {
            if (!names.includes(col)) {
                try {
                    await conn.query(`ALTER TABLE tbl_admissions ADD COLUMN ${col} ${def}`);
